...
const BlockActions: React.FunctionComponent<{
  ...
  const completeBlockByGuest = useStoreActions(actions => actions.components.blockPage.block.completeBlockByGuest);
    ...
    <>
          ...
          <Button
            ...
            onClick={() => {
              ...
              if(session.isLoggedIn) {
                ...
                completeBlockByGuest({block, module, doneCallback: completeBlockDoneCallback});
              }
            }}
          >
            Продолжить
      ...
      <div className={styles.actions}>
        ...
        {!block.nextBlockSlug && !!block.courseSlug &&
          <Button
            className="btn_secondary"
            aria-label={`Вернуться к программе курса`}
            onClick={() => {
