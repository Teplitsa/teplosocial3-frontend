...
const QuizActions: React.FunctionComponent<{
  ...
  const activeQuestionIndex = useStoreState(
    (state) => state.components.blockPage.quiz.activeQuestionIndex
  );
  ...
  const questions = useStoreState(
    (state) => state.components.blockPage.quiz.questions
  );
  const setActiveQuestionIndex = useStoreActions(
    (actions) => actions.components.blockPage.quiz.setActiveQuestionIndex
  );
  const addAnsweredQuestion = useStoreActions(
    (actions) => actions.components.blockPage.quiz.addAnsweredQuestion
  );
  const startQuizByUser = useStoreActions(
    (actions) => actions.components.blockPage.quiz.startQuizByUser
  );
  const checkQuizUserAnswers = useStoreActions(
    (actions) => actions.components.blockPage.quiz.checkQuizUserAnswers
  );
  const completeQuizByUser = useStoreActions(
    (actions) => actions.components.blockPage.quiz.completeQuizByUser
  );
  ...
  const answeredQuestions = useStoreState(
    (state) => state.components.blockPage.quiz.answeredQuestions
  );
  ...
  // adaptest
  const loadModuleListByCourseCompletedByAdaptest = useStoreActions(
    (actions) =>
      actions.components.blockPage.course
        .loadModuleListByCourseCompletedByAdaptest
  );
  ...
  useEffect(() => {
    ...
    completeQuizByUser({
      ...
      module,
      user,
      doneCallback: quizCompletedByUserHandler,
    });
  ...
  useEffect(() => {
    ...
    if (questions.length > answeredQuestions.length) {
      ...
      notAnsweredQuestionIndex = questions.findIndex((q) => {
        ...
        const findResult = answeredQuestions.find(
          (answeredQuestionId) => answeredQuestionId === q.id
        );
    ...
    if (notAnsweredQuestionIndex >= 0) {
      ...
      checkQuizUserAnswers({
        quiz,
        module,
        user,
        doneCallback: quizUserAnswersCheckedHandler,
      });
    }
  ...
  const forwardHandler = () => {
    if (_.isEmpty(answeredQuestions)) {
      startQuizByUser({
        ...
        module,
        user,
        doneCallback: quizStartedHandler,
      });
    }
  ...
  return (
    <div
      className={convertObjectToClassName({
        [styles.actions]: !["checklist", "quiz"].includes(quiz.quizType),
        [styles["single-action"]]: ["checklist", "quiz"].includes(
          quiz.quizType
        ),
      })}
    >
        ...
        <>
          ...
          {["checklist", "quiz"].includes(quiz.quizType) &&
            !(isQuizCompleted || block.isCompleted) && (
              <Button
                className={convertObjectToClassName({
                  btn_primary: true,
                  [styles["single-action__btn"]]: true,
                })}
                aria-label={`Ответить на вопрос ${question.title.rendered}`}
                onClick={forwardHandler}
          ...
          {!isQuizCompleted && block.isCompleted && (
            <Button
              className={convertObjectToClassName({
                btn_secondary: true,
                [styles["single-action__btn"]]: ["checklist", "quiz"].includes(
                  quiz.quizType
                ),
              })}
