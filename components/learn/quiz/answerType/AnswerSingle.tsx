...
const AnswerSingle: React.FunctionComponent = (): ReactElement => {
  ...
  return (
    <>
      {question.answerData.map(answer => {
        ...
        return (
          <div key={`InputRadio-${question.id}-Answer${answer.answer}`} className={isSelected ? styles.choiceOptionSelected : styles.choiceOption}>
            <InputRadio
              label={answer.answer}
              ...
              checked={isSelected}
              onChange={(event) => {
                setUserAnswer({questionId: question.id, answerValue: answer.answer})
              }}
            />                
          </div>
        );
