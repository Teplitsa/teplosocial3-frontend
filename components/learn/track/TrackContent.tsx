...
const TrackContent: React.FunctionComponent<{
  ...
  const courseList = useStoreState(state => state.components.trackPage.courseList);
  const courseListModuleList = useStoreState(state => state.components.trackPage.courseListModuleList);
  const requestModuleListByCourse = useStoreActions(actions => actions.components.trackPage.requestModuleListByCourse);
    ...
    <div className={stylesTrack.content}>
      ...
      <div className={styles.list}>
        {courseList.map((course) => {
          return (
            <CourseListItem key={`TrackCourse${course.id}`} course={course} moduleList={courseListModuleList[course.id]} />
          )
        })}
      </div>
        ...
        <div className={stylesTrack.startAction}>
          ...
          {!trackStartLoading &&
            <Button
              ...
              aria-label={`${track.isStarted ? "Продолжить" : "Начать"} обучение по треку «${track.title.rendered}»`}
              onClick={() => {
