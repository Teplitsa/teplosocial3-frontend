...
const LearnProgress: React.FunctionComponent = (): ReactElement => {
  ...
  useEffect(() => {
    if(!coursePage || !coursePage.course || !coursePage.course.id || !page || !page.slug) {
      return;
    }
  ...
  useEffect(() => {
    if(!blockPage || !blockPage.block || !blockPage.block.id || !page || !page.slug) {
      return;
    }
  ...
  useEffect(() => {
    if(!trackPage || !trackPage.track || !trackPage.track.id || !page || !page.slug) {
      return;
    }
    ...
    setProgressValue(
      trackPage.track.numberOfBlocks 
        ? Math.floor(trackPage.track.numberOfCompletedBlocks * 100 / trackPage.track.numberOfBlocks) 
        : 0
    );
  }, [trackPage, page])
  ...
  return (
    <div className={styles.container}>
      <ProgressBar {...progress}>
        <a className={styles.link} href={C.TEPLO_CONTACTS.TELEGRAM} target="_blank">
          <img src={imageChat} />
          <span>Чат</span>
        </a>
      </ProgressBar>
